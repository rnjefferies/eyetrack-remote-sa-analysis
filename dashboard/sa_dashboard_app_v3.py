# ===========================================================================
# sa_dashboard_app_v3.py: SA Operator Monitor (live demo dashboard)
# ===========================================================================
...
#
#   Run (from the repo root):  python dashboard/sa_dashboard_app_v3.py   then open http://127.0.0.1:8051
...
OP = pd.read_csv('dashboard/SA_Dashboard_Ops.csv')
M  = pd.read_csv('dashboard/SA_Dashboard_Meta.csv').iloc[0]
...
# ---------------------------------------------------------------------------
# Confidence-prompt policy: SCHEDULED checkpoints (start + mid) + Red warnings.
...
#   * a HIGH reading (>=6) at a Red warning raises the overconfidence flag.
# Baseline-relative interpretation (reading a score as a drop from the operator's own
# norm) is deferred, it needs more per-operator training data.
...
def _augment_confidence_v2(df):
    ...
    # Request a rating only when the Red warning PERSISTS (>=2 probes in a row), not on a
    # one-off flicker: fewer, cleaner prompts, and it sharpens the confirmed signal
    # (sustained Red is more genuinely risky).
...
app.layout = html.Div(style={'backgroundColor': C_BG, 'minHeight': '100vh',
    ...
    html.Div([
        ...
        html.Div('A live demo of the situational-awareness monitor. For each operator it shows two objective '
                 'risk dials (error and latency), a triggered manual confidence check, a supervisor triage '
                 'board, and a drill-in for the selected operator. The dials read the RECENT STATE: a short '
                 'trailing average (EWMA), shown with a trend arrow and a persistence badge that together '
                 'suggest whether the operator attention is dipping, and whether it is sustained. '
                 'The dials are a soft early-warning aid, not a verdict (per-probe ROC about 0.65 to 0.7). At '
                 'scheduled points, and when a Red warning persists, the operator rates their own confidence: '
                 'a low rating confirms the alert (about two-thirds of confirmed warnings fail), a high rating '
                 'flags overconfidence. '
                 'An overt control lapse forces Red outright.',
