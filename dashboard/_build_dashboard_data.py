...
#   dashboard/SA_Dashboard_Data.csv  (probe-level: separate dials, fail/lapse, session order)
#   dashboard/SA_Dashboard_Ops.csv   (operator-level: dial means, base-rate LIFT, dominant
...
# === Eyes-forward road-gaze, diffuse index, and angular saccade units ===
# (the examiner route CSVs ship with the original road-gaze; this derives the corrected
#  columns at load time, matching notebooks 01/03/04/05/06.)
...
for _p in ['Before', 'During']:
    if f'{_p}_Scanpath_Rate_px_s' in df.columns:
        df[f'{_p}_Scanpath_Rate_deg_s'] = df[f'{_p}_Scanpath_Rate_px_s'] * _DEG_PER_PX
    if f'{_p}_Mean_Saccadic_Velocity' in df.columns:
        df[f'{_p}_Mean_Saccadic_Velocity_deg_s'] = df[f'{_p}_Mean_Saccadic_Velocity'] * _DEG_PER_PX
...
def oof(y, kind):
    if kind == 'xgb':
        ...
        m = xgb.XGBClassifier(scale_pos_weight=w, eval_metric='logloss', random_state=42, n_jobs=1,
                              max_depth=3, learning_rate=0.03, n_estimators=100)  # nested-CV winner under eyes-forward road-gaze
    elif kind == 'rf':
        m = RandomForestClassifier(class_weight='balanced', n_estimators=100, max_depth=2, random_state=42)  # nested-CV winner under eyes-forward road-gaze
    else:
...
df[keep].to_csv('dashboard/SA_Dashboard_Data.csv', index=False)
op.to_csv('dashboard/SA_Dashboard_Ops.csv', index=False)
meta.to_csv('dashboard/SA_Dashboard_Meta.csv', index=False)

print("WROTE dashboard/SA_Dashboard_Data.csv (%d probes), dashboard/SA_Dashboard_Ops.csv (%d ops), dashboard/SA_Dashboard_Meta.csv"
      % (len(df), len(op)))
